 ...
 * Default constructor
 */
BuyNow::BuyNow(): fornecedores(fornecedores){
    fornecedores.makeEmpty();
}
...
BuyNow::BuyNow(vector<LojaFisica> &lf, LojaOnline &lo, int stockOk, int stockMin):
        lojasFisicas(lf), lojaOnline(lo), stockOk(stockOk), stockMin(stockMin), fornecedores(fornecedores)
        {
    fornecedores.makeEmpty();
}
...
BuyNow::BuyNow(vector<LojaFisica> &lf, LojaOnline &lo, vector<Transferencia *> tranferencias, int stockOk, int stockMin):
        ...
        fornecedores(fornecedores) { fornecedores.makeEmpty(); }
    ...
    for (auto i : produtos){
        if (lojaOnline.getStockOnline(i) < stockMin){
            ...
            if (lojasFisicas[idxLojaMaiorStock].getStockFisico(i) - repor > stockOk){   //a loja tem stockSuficiente
                ...
                BSTItrIn<Fornecedor*> it(fornecedores);
                Fornecedor* forn;
                while(!it.isAtEnd()){   //search for fornecedor
                    if (it.retrieve()->getQuantidade() >= repor){
                        forn = it.retrieve();
                        break;
                    }
                }
 ...
 */
BST<Fornecedor *> BuyNow::getFornecedores() const { return fornecedores; }
